...
def check_reminders():
    ...
    for reminder_id, title, message, reminder_time in reminders:
...
color:#d1d5db;
font-size:13px;">
Reminder generated by <strong>MeetPilot AI</strong>
</p>
...
"""
            ...
            if success:
                ...
                print(f"✅ Reminder sent: {reminder_id}")

            else:
                print(f"❌ Failed reminder: {reminder_id}")
...
def process_audio():
    try:
        ...
        cur.execute("""
            INSERT INTO meetings (user_id, title, meeting_date, status)
            VALUES (%s, %s, %s, %s)
            RETURNING meeting_id
        """, (user_id, title, now_ist, "processing"))
...
def process_audio_background(filepath, meeting_id):
    try:
        start_time = time.time()
        ...
        # 🤖 STEP 2: GROQ (ONLY summary, key points, decisions)
        groq_data = generate_meeting_data(transcript)

        summary = groq_data.get("insight", "")
        key_points = groq_data.get("key_points", [])
        decisions = groq_data.get("decisions", [])

        # 🧠 STEP 3: YOUR MODEL (ONLY action items)
        raw_actions = get_action_items_from_model(transcript)
        action_items = format_action_items(raw_actions)
        ...
        cur.execute("""
            UPDATE meetings
            SET transcript=%s, processing_time=%s, status=%s
            WHERE meeting_id=%s
        """, (transcript, processing_time, "completed", meeting_id))

        # ✅ INSERT summary (ONLY ONCE)
        cur.execute("""
            INSERT INTO summaries (meeting_id, summary_text, key_points, decisions, action_items)
            VALUES (%s, %s, %s, %s, %s)
        """, (
            meeting_id,
            summary,
...
def send_email_route():
    ...
    # ✅ PROFESSIONAL EMAIL-CLIENT SAFE HTML
...
# Format date nicely
    ...
    color:#d1d5db;
    font-size:13px;">
    Generated automatically by <strong>MeetPilot AI</strong>
    </p>
...
@app.route("/create-reminder/<int:user_id>", methods=["POST"])
def create_reminder(user_id):
    try:
        ...
        # 1. create reminder
        cur.execute("""
            INSERT INTO meeting_reminders (title, message, reminder_time, created_by)
            VALUES (%s, %s, %s, %s)
            RETURNING reminder_id
        """, (title, message, reminder_time, user_id))

        reminder_id = cur.fetchone()[0]

        # 2. insert recipients
        for participant_id in selected_users:
            cur.execute("""
                INSERT INTO reminder_recipients (reminder_id, user_id)
                VALUES (%s, %s)
            """, (reminder_id, participant_id))
...
def get_meeting_summary(meeting_id):
    try:
        ...
        cur.execute("""
            SELECT meeting_id, title, status, meeting_date, processing_time
        ...
        # ✅ Use tuple indices, not dict keys
        meeting_id_db, title, status, meeting_date, processing_time = meeting

        if status != "completed":
            return jsonify({"meeting_id": meeting_id, "status": "processing"}), 200

        cur.execute("""
            SELECT summary_text, key_points, action_items, decisions
            FROM summaries WHERE meeting_id = %s
        """, (meeting_id,))
        summary = cur.fetchone()
        ...
        summary_text, key_points, action_items, decisions = summary

        return jsonify({
            "meeting_id": meeting_id,
            "status": "completed",
            "title": title,
            "created_at": meeting_date.isoformat() if meeting_date else None,
            "processing_time": processing_time,
            "insight": summary_text,
            "key_points": key_points if isinstance(key_points, list) else json.loads(key_points or "[]"),
            "action_items": action_items if isinstance(action_items, list) else json.loads(action_items or "[]"),
            "decisions": decisions if isinstance(decisions, list) else json.loads(decisions or "[]"),
        })

    except Exception as e:
        print("❌ meeting-summary error:", str(e))  # ✅ always log the actual error
        return jsonify({"error": str(e)}), 500
